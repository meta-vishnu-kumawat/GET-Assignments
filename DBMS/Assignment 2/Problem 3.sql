...
-- OrderTotal is calculated as the sum of (Price * Quantity) for each order.
SELECT 
    O.OrderID AS Id,
    O.OrderDate,
...
-- The most expensive orders are determined by the total amount (Price * Quantity).
SELECT 
    O.OrderID AS Id,
    O.OrderDate,
    SUM(P.Price * OI.Quantity) AS OrderTotal
...
-- (Only 'Shopper' users are considered.)
SELECT 
    U.UserID,
    U.Username,
    U.Email
...
-- Only 'Shopper' users are included.
SELECT
    U.UserID,
    U.Username,
    O.OrderID,
...
-- 3.6 Display list of order items which are in 'Shipped' state for a particular Order Id.
...
-- This query displays order item details along with the order date for products within the price range.
SELECT 
    OI.OrderItemID,
    O.OrderDate,
