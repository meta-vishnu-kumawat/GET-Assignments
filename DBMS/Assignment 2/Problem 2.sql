...
SELECT P.ProductID AS Id,P.Name AS Title,C.Name AS CategoryTitle,P.Price
FROM Product P
...
SELECT P.ProductID AS Id,P.Name AS Title,P.Price
FROM Product P
...
-- If Category is a top category then Parent Category Title should display 'Top Category'.
SELECT C.CategoryID AS Id,C.Name AS Title,
...
-- For example, for category Title "Mobile" (using a LIKE clause to match "Mobile Phones").
SELECT P.Name AS ProductTitle,P.Price,P.Description
FROM Product P
JOIN ProductCategory PC ON P.ProductID = PC.ProductID
JOIN Category C ON PC.CategoryID = C.CategoryID
